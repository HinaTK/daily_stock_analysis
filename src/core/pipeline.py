...
class StockAnalysisPipeline:
    ...
    def analyze_stock(
        self, code: str, report_type: ReportType
        ...
        try:
            ...
            if context is None:
                logger.warning(
                    f"[{code}] 无法获取历史行情数据，将仅基于新闻和实时行情分析"
                )
                from datetime import date
            ...
            if result and trend_result:
                self._merge_trend_quant_plan(result, trend_result)
    ...
    def _enhance_context(
        ...
        if trend_result:
            enhanced["trend_analysis"] = {
                ...
                "entry_price": trend_result.entry_price,
                "stop_loss_price": trend_result.stop_loss_price,
                "target_price": trend_result.target_price,
                "recommended_position_pct": trend_result.recommended_position_pct,
                "risk_reward_ratio": trend_result.risk_reward_ratio,
                "atr_14": trend_result.atr_14,
                "atr_stop_loss": trend_result.atr_stop_loss,
                "timeframe_alignment": trend_result.timeframe_alignment,
                "resonance_count": trend_result.resonance_count,
                "resonance_passed": trend_result.resonance_passed,
                "signal_age_days": trend_result.signal_age_days,
                "signal_valid": trend_result.signal_valid,
                "main_fund_net_inflow": trend_result.main_fund_net_inflow,
                "main_fund_inflow_ratio": trend_result.main_fund_inflow_ratio,
                "northbound_net_inflow": trend_result.northbound_net_inflow,
    ...
    def _extract_intraday_df(self, context: Dict[str, Any]) -> Optional[pd.DataFrame]:
        """从上下文中提取 30 分钟级别数据（如存在）。"""
        intraday_keys = ["intraday_30m", "intraday_data", "raw_intraday_data"]
    ...
    def _fetch_intraday_30m(self, code: str) -> Optional[pd.DataFrame]:
        ...
        candidates = [
            ("get_intraday_data", {"stock_code": code, "period": "30m", "days": 5}),
            ("get_minute_data", {"stock_code": code, "period": "30", "days": 5}),
            ("get_kline_data", {"stock_code": code, "period": "30m", "days": 5}),
        ]

        for method_name, kwargs in candidates:
            if not hasattr(self.fetcher_manager, method_name):
                continue

            method = getattr(self.fetcher_manager, method_name)
            try:
                df = method(**kwargs)
                if isinstance(df, pd.DataFrame) and not df.empty:
                    required = {"date", "open", "high", "low", "close", "volume"}
                    if required.issubset(df.columns):
                        logger.info(f"[{code}] 30分钟数据可用，共 {len(df)} 条")
                        return df
            except Exception as e:
                logger.debug(f"[{code}] 获取30分钟数据失败({method_name}): {e}")

        return None

    def _merge_trend_quant_plan(
        self, result: AnalysisResult, trend: TrendAnalysisResult
    ) -> None:
        """将趋势分析器输出注入到 dashboard，供通知模板直接渲染。"""
        ...
        quant_plan = {
            ...
            "entry_price": round(trend.entry_price, 2),
            "stop_loss_price": round(trend.stop_loss_price, 2),
            "target_price": round(trend.target_price, 2),
            "recommended_position_pct": round(trend.recommended_position_pct, 1),
            "risk_reward_ratio": trend.risk_reward_ratio,
            "atr_14": round(trend.atr_14, 4),
            "atr_stop_loss": round(trend.atr_stop_loss, 4),
            "timeframe_alignment": trend.timeframe_alignment,
            "timeframe_notes": trend.timeframe_notes,
            "resonance_passed": trend.resonance_passed,
            "resonance_count": trend.resonance_count,
            "signal_valid": trend.signal_valid,
            "signal_age_days": trend.signal_age_days,
            "main_fund_net_inflow": trend.main_fund_net_inflow,
            "main_fund_inflow_ratio": trend.main_fund_inflow_ratio,
            "northbound_net_inflow": trend.northbound_net_inflow,
            "boll_position": trend.boll_position,
            "kdj_status": trend.kdj_status,
            "obv_trend": trend.obv_trend,
        }
        result.dashboard["quant_plan"] = quant_plan
