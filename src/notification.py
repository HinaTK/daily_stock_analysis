...
class NotificationService:
    ...
    def _append_quant_plan(
        self, lines: List[str], result: AnalysisResult, compact: bool = False
    ) -> None:
        """追加量化交易清单（来自 dashboard.quant_plan）。"""
        dashboard = result.dashboard if hasattr(result, "dashboard") else None
        quant = dashboard.get("quant_plan", {}) if isinstance(dashboard, dict) else {}
        if not quant:
            return

        if compact:
            lines.append(
                f"📐 量化: 入场 {quant.get('entry_price', 'N/A')} | 止损 {quant.get('stop_loss_price', 'N/A')} | "
                f"目标 {quant.get('target_price', 'N/A')} | 仓位 {quant.get('recommended_position_pct', 'N/A')}%"
            )
            lines.append(
                f"🧮 RR 1:{quant.get('risk_reward_ratio', 'N/A')} | 共振 {quant.get('resonance_count', 'N/A')}项 | "
                f"信号{('有效' if quant.get('signal_valid', True) else '衰减')}"
            )
            lines.append("")
            return
        ...
        lines.extend(
            [
                ...
                "| 项目 | 数值 |",
                "|------|------|",
                f"| 信号等级 | {quant.get('buy_signal', 'N/A')} ({quant.get('signal_score', 'N/A')}分) |",
                f"| 入场价 | {quant.get('entry_price', 'N/A')} |",
                f"| 止损价 | {quant.get('stop_loss_price', 'N/A')} |",
                f"| 目标价 | {quant.get('target_price', 'N/A')} |",
                f"| 建议仓位 | {quant.get('recommended_position_pct', 'N/A')}% |",
                f"| 风险收益比 | 1:{quant.get('risk_reward_ratio', 'N/A')} |",
                f"| ATR(14) | {quant.get('atr_14', 'N/A')} |",
                f"| ATR止损距离 | {quant.get('atr_stop_loss', 'N/A')} |",
                f"| 多周期共振 | {'通过' if quant.get('timeframe_alignment') else '未通过'} |",
                f"| 多信号共振 | {'通过' if quant.get('resonance_passed') else '未通过'} ({quant.get('resonance_count', 'N/A')}项) |",
                f"| 信号衰减 | {'有效' if quant.get('signal_valid', True) else '失效'} (龄期 {quant.get('signal_age_days', 'N/A')} 天) |",
                f"| 主力净流入 | {quant.get('main_fund_net_inflow', 'N/A')} |",
                f"| 主力净占比 | {quant.get('main_fund_inflow_ratio', 'N/A')}% |",
                f"| 北向净流入 | {quant.get('northbound_net_inflow', 'N/A')} |",
                f"| 布林带位置 | {quant.get('boll_position', 'N/A')} |",
                f"| KDJ状态 | {quant.get('kdj_status', 'N/A')} |",
                f"| OBV趋势 | {quant.get('obv_trend', 'N/A')} |",
                "",
            ]
        )

        notes = quant.get("timeframe_notes", [])
        if notes:
            lines.append("**多周期备注**:")
            for note in notes:
                lines.append(f"- {note}")
    ...
    def generate_dashboard_report(
        ...
        for result in sorted_results:
            ...
            # 量化交易清单（新增）
            self._append_quant_plan(report_lines, result, compact=False)
    ...
    def generate_wechat_dashboard(self, results: List[AnalysisResult]) -> str:
        ...
        for result in sorted_results:
            ...
            # 量化清单（精简）
            self._append_quant_plan(lines, result, compact=True)
    ...
    def generate_single_stock_report(self, result: AnalysisResult) -> str:
        ...
        # 量化交易清单（单股版）
        self._append_quant_plan(lines, result, compact=False)
    ...
    def _send_discord_webhook(self, content: str) -> bool:
        ...
        try:
            ...
            response = requests.post(
                self._discord_config["webhook_url"], json=payload, timeout=10
            )
