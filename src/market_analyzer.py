...
class MarketAnalyzer:
    ...
    def _get_sector_rankings(self, overview: MarketOverview):
        ...
        try:
            ...
            # Fallback: use local sector fetcher (AkShare based) when provider rankings are unavailable
            if not top_sectors and not bottom_sectors:
                try:
                    from src.sector.sector_fetcher import SectorFetcher

                    fetcher = SectorFetcher()
                    ranked = fetcher.get_hot_sectors(limit=15)
                    ranked = [
                        {
                            "name": s.get("name", ""),
                            "change_pct": float(s.get("change_pct", 0) or 0.0),
                        }
                        for s in ranked
                        if s.get("name")
                    ]
                    ranked.sort(key=lambda x: x.get("change_pct", 0.0), reverse=True)
                    top_sectors = ranked[:5]
                    bottom_sectors = (
                        list(reversed(ranked[-5:])) if len(ranked) >= 5 else []
                    )
                except Exception as e:
                    logger.debug(f"[大盘] Fallback 板块排行失败: {e}")
    ...
    def _build_review_prompt(self, overview: MarketOverview, news: List) -> str:
        ...
        has_stats = bool(
            ...
            or overview.flat_count
            or overview.limit_up_count
            or overview.limit_down_count
